        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
          {galleryImages.map((img, idx) => (
            <button
              ...
              className="group relative aspect-[4/3] rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-900 shadow-md hover:shadow-2xl hover:scale-105 hover:border-amber-500 transition-all cursor-pointer flex flex-col justify-end p-3 text-left"
            >
              <img 
                src={img.url} 
                alt={img.title} 
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/20 to-transparent group-hover:from-amber-950/90 transition-colors"></div>
              
              <div className="relative z-10 space-y-0.5">
                <span className="inline-flex items-center gap-1 text-[10px] font-mono font-bold text-amber-400">
                  <Maximize2 className="w-3 h-3 text-amber-400" /> Lihat Foto
                </span>
                <h4 className="font-display text-xs font-bold text-white line-clamp-1 group-hover:text-amber-300">
                  {img.title}
                </h4>
              </div>
            </button>
            ...
            {(() => {
              ...
              return (
                <div className="p-5 bg-slate-900 text-white border-t border-slate-800 space-y-1">
                  <h4 className="font-display text-lg font-bold text-amber-400">{matched.title}</h4>
